...
double calculate_weighted_sse(SparseMatrix *R, double **P, double **Q,
  ...
  for (int i = 0; i < num_users; i++) {
    ...
    while (node) {
      ...
      // Assuming node->original_rating is the original integer rating before
      // normalization
      int original_rating = (int)round((node->rating * stddev) + mean);

      if (original_rating >= 1 && original_rating <= 5) {
        sse += weights[original_rating] * error *
               error; // Apply weight based on original rating
      } else {
...
double calculate_sse(SparseMatrix *R, double **P, double **Q, int num_users,
  ...
  for (int i = 0; i < num_users; i++) {
    ...
    while (node) {
      ...
      // if (count < 10) {
      //     printf("User %d, Item %d: Actual = %f, Predicted = %f, Error =
...
void train_model(SparseMatrix *R, double **P, double **Q, int num_users,
  ...
  for (int epoch = 0; epoch < epochs; epoch++) {
    ...
    if (epoch % 100 == 0) {
      double sse = calculate_weighted_sse(R, P, Q, num_users, num_items,
                                          num_factors, weights, mean, stddev);
      ...
      for (int i = 0; i < num_users && i < 5; i++) {
        ...
        while (node) {
          double prediction =
              predict_rating(P, Q, i, node->song_id, num_factors);
          double error = node->rating - prediction;
          // printf("User %d, Item %d: Actual = %f, Predicted = %f, Error =
          // %f\n", i, node->song_id, node->rating, prediction, error);
          node = node->next;
...
void recommend_items(double **P, double **Q, int user_id, int num_items,
  ...
  for (int item_id = 0; item_id < num_items; item_id++) {
    double normalized_pred =
        predict_rating(P, Q, user_id, item_id, num_factors);
